...
|---|---|---|---|
| 2026-08-18 | [frame-align-null](2026-08-18_frame-align-null/) | **Alignment needs ~500 ms — one AWARE hop (50 ms) is unalignable at 5.1%.** And **the bake-off winner loses at frame level**: `aof` beats `gcc_phat` at every length ≥250 ms (80.3% vs 53.8% useful-accept at 250 ms). Neither method has a reject option — on 7,200 unrelated pairs both always returned an offset, so confidence is the only guard, and it is near chance (AUC 0.592) below 100 ms. `gcc_phat` needs a per-frame-length threshold (48→28); `aof` holds one global τ≈5. **Predictions 3 and 4 refuted** — 77% of short-frame failures are on *voiced* audio, not silence. Phase 1 (clean, native refs) only. See [README.md](2026-08-18_frame-align-null/README.md) | **complete** (phase 1; phase 2 pending) |
