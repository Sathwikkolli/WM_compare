...
|---|---|---|---|
| 2026-08-28 | [informed-detection](2026-08-28_informed-detection/) | *(planned)* Does an aligned original recover watermarks blind detection loses? **The A/B's two failures are the wrong test cases** — damage-control showed they are destroyed audio (PESQ 1.04-1.32). Phase A first screens attacks for the ones that break detection while audio stays **usable**, plotting detection against *quality* rather than attack strength. Lead candidate already in the repo: a music bed at **+4 dB SNR**, where confidence drops to 0.402 but bit accuracy is still **0.925**. **DNSMOS is primary, not PESQ** — PESQ would wrongly file a music mix as destroyed. Predictions registered. See [README.md](2026-08-28_informed-detection/README.md) | **planned** |
